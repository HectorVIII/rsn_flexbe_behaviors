...
from rsn_interfaces.action import DetectHand


class StartHandDetectionState(EventState):
    """
    Runs hand detection once and blocks until a stable hand pose is published.

    Rewritten from a Trigger-service delegate to an ActionState so the
    outcome only fires AFTER zed_hand_node has actually emitted a fresh
    pose on /right_hand_pose_base. Previously the delegate returned SUCCESS
    as soon as the ZED opened, which let MOVE_TO_HAND consume a stale pose
    ...
    #> response_message string  Action response message.
    ...
    def __init__(self, action_topic='/start_hand_detection', timeout_sec=30.0):
    ...
    def execute(self, userdata):
        ...
        if status != GoalStatus.STATUS_SUCCEEDED:
            Logger.logwarn(
                f'Hand detection action ended with status={status}: '
                f'{result.message}'
            )
            return 'failed'

        if result.success:
            Logger.loginfo(f'Hand detection succeeded: {result.message}')
            return 'done'

        Logger.logwarn(f'Hand detection failed: {result.message}')
        return 'failed'
    ...
    def _send_goal_if_available(self, userdata):
        ...
        self._sent = True
        Logger.loginfo(
            f'Sent hand detection action goal (timeout={self._timeout_sec:.1f}s).'
        )
